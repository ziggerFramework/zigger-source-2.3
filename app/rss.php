...
class index {
    ...
    public function init()
    {
        ...
        if ($arr['use_rss'] != 'Y') {
            Func::core_err('RSS 발행 기능이 활성화 되지 않았습니다.');
        }

        if (!$rss_json) {
            Func::core_err('RSS 발행 구문이 올바르지 않습니다.');
        }
            ...
            if ($sql->getcount() > 0) {
                do {
                    ...
                    $rss_data[$sql->fetch('regdate')] = array(
                        ...
                        'link' => $value['link'].'?mode=view&amp;read='.$sql->fetch('idx'),
                        'title' => $value['title'],
                        'data' => $sql->fetchs()
                    );
        ...
        $pubDate = date('r');
        if (count($rss_data) > 0) {
            $pubDate = date('r', strtotime($rss_data[key($rss_data)]['pubDate']));
        }
